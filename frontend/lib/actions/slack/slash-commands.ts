...
export async function processSlashCommand(
  ...
  if (command !== "/hipocap") {
    return {
      ...
      text: "Unknown command. Please use `/hipocap subscribe <event_name>` or `/hipocap unsubscribe [event_name]`",
    };
  }
  ...
  if (subcommand === "help" || !subcommand) {
    return {
      response_type: "ephemeral",
      text: "📚 *Hipocap Slash Commands*\n\n• `/hipocap subscribe <event_name>` - Subscribe this channel to an event\n• `/hipocap unsubscribe [event_name]` - Unsubscribe from an event (or all events if no name provided)\n• `/hipocap help` - Show this help message",
    };
  }
  ...
  if (subcommand === "subscribe") {
    ...
    if (!eventName) {
      return {
        response_type: "ephemeral",
        text: "❌ Please provide an event name. Usage: `/hipocap subscribe <event_name>`",
      };
    }
...
async function handleSubscribeCommand(
  ...
  try {
    ...
    if (!integration) {
      return {
        response_type: "ephemeral",
        text: "❌ Hipocap is not connected to this workspace. Please install the Hipocap app first.",
      };
    }
