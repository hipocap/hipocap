...
export async function getTraces(input: z.infer<typeof GetTracesSchema>): Promise<{ items: TraceRow[] }> {
  ...
  let spanHits: { trace_id: string; span_id: string }[] = [];
  try {
    spanHits = search
      ? await searchSpans({
        projectId,
  ...
  let items: TraceRow[] = [];
  try {
    items = await executeQuery<TraceRow>({ query: mainQuery, parameters: mainParams, projectId });
  } catch (error) {
    console.error("Error executing traces query:", error);
