...
export async function getProviderApiKey(projectId: string, provider: Provider): Promise<string> {
  ...
  if (!key) {
    throw new Error(`No matching provider key found for ${provider} (${apiKeyName})`);
  }

  if (!key.nonceHex || !key.value) {
    throw new Error(`Provider key ${apiKeyName} is missing nonce or encrypted value`);
  }

  try {
    const decodedKey = await decodeApiKey(key.name, key.nonceHex, key.value);
    
    if (!decodedKey || decodedKey.trim().length === 0) {
      throw new Error(`Decoded API key for ${provider} is empty`);
    }
    
    return decodedKey;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to get provider API key for ${provider}: ${errorMessage}`);
  }
}
