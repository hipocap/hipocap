...
const nextConfig: NextConfig = {
  ...
  serverExternalPackages: ["@lmnr-ai/lmnr", "@sentry/nextjs", "libsodium-wrappers"],
  output: "standalone",
  async headers() {
    return [
      {
  ...
  webpack: (config, { isServer }) => {
    ...
    if (isServer) {
      ...
      // Mark libsodium-wrappers as external to avoid bundling issues
      config.externals.push("libsodium-wrappers");
    }
    return config;
  },
  turbopack: {
