...
export const MAX_TREE_VIEW_WIDTH = 650;
...
interface TraceViewStoreActions {
  ...
  setAutoOpenEventsTab: (autoOpen: boolean) => void;
  ...
  createStore<TraceViewStore>()(
    persist(
      (set, get) => ({
        ...
        autoOpenEventsTab: false,

        setHasBrowserSession: (hasBrowserSession: boolean) => set({ hasBrowserSession }),
        setSelectedEventId: (eventId?: string) => set({ selectedEventId: eventId }),
        setAutoOpenEventsTab: (autoOpen: boolean) => set({ autoOpenEventsTab: autoOpen }),
        setTrace: (trace) => {
        ...
        getMinimapSpans: () => {
          ...
          if (trace && spans.length > 0) {
            // Calculate actual span time range to match how spans are positioned
            const allStartTimes = spans.map(s => new Date(s.startTime).getTime());
            const allEndTimes = spans.map(s => new Date(s.endTime).getTime());
            const minTime = Math.min(...allStartTimes);
            const maxTime = Math.max(...allEndTimes);
            const actualDuration = maxTime - minTime;
            return transformSpansToMinimap(spans, actualDuration);
          }
          return [];
        },
        getListMinimapSpans: () => {
          ...
          if (trace) {
            ...
            if (listSpans.length > 0) {
              // Calculate actual span time range to match how spans are positioned
              const allStartTimes = listSpans.map(s => new Date(s.startTime).getTime());
              const allEndTimes = listSpans.map(s => new Date(s.endTime).getTime());
              const minTime = Math.min(...allStartTimes);
              const maxTime = Math.max(...allEndTimes);
              const actualDuration = maxTime - minTime;
              return transformSpansToFlatMinimap(listSpans, actualDuration);
            }
          }
          return [];
        },
        ...
        setSelectedSpan: (span) => {
          // If selecting a virtual Hipocap event span, select its parent instead
          if (span?.attributes?.["hipocap.is_event_span"]) {
            const parentSpanId = span.parentSpanId;
            if (parentSpanId) {
              const parentSpan = get().spans.find((s) => s.spanId === parentSpanId);
              if (parentSpan) {
                set({ selectedSpan: parentSpan });
                return;
              }
            }
          }
        ...
        selectSpanById: (spanId: string) => {
          ...
          if (span && !span.pending) {
            // If selecting a virtual Hipocap event span, select its parent instead
            if (span.attributes?.["hipocap.is_event_span"] && span.parentSpanId) {
              const parentSpan = get().spans.find((s) => s.spanId === span.parentSpanId);
              if (parentSpan) {
                set({ selectedSpan: parentSpan });
                const spanPath = parentSpan.attributes?.["lmnr.span.path"];
