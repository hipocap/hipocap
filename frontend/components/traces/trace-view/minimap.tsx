...
function Minimap({ onSpanSelect }: Props) {
  ...
  // Calculate actual span time range (min to max) to match how spans are positioned
  const actualTimeRange = useMemo(() => {
    if (!spans || spans.length === 0) {
      const traceDuration = trace?.endTime && trace?.startTime 
        ...
        : 0;
      return { minTime: trace?.startTime ? new Date(trace.startTime).getTime() : 0, duration: traceDuration };
    }
    ...
    return { minTime, duration };
  }, [spans, trace?.startTime, trace?.endTime]);

  const traceDuration = useMemo(
    () => actualTimeRange.duration,
    [actualTimeRange.duration]
  );
  ...
  const timeMarkers = useMemo(() => {
    ...
    const markers = [];
    const totalSeconds = traceDuration / 1000; // Keep as decimal for accuracy
    const totalSecondsRounded = Math.ceil(totalSeconds);

    // Generate markers up to and including the final second that covers the duration
    for (let seconds = 0; seconds <= totalSecondsRounded; seconds += timeMarkerInterval) {
      markers.push({
        seconds,
        label: `${seconds}s`,
      });
  ...
  return (
    <div className="h-full flex-none max-w-16 w-fit z-30 border-l">
      <div
        ...
        style={{
          minHeight: `${(traceDuration / 1000) * pixelsPerSecond + 2}px`,
        }}
      >
        <div
        ...
        <div 
          ...
          style={{
            height: `${(traceDuration / 1000) * pixelsPerSecond}px`,
          ...
          {minimapSpans.map((span, index) => {
            ...
            return (
              <div
                ...
                key={span.spanId}
                className={cn(
                  "bg-background duration-100 transition-opacity",
                  tab === "reader"
                    ? isVisible
                      ? "opacity-100"
