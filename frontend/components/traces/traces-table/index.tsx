...
import TracesMetrics from "@/components/traces/traces-metrics";
...
export default function TracesTable({ showChart = true, showTable = true }: { showChart?: boolean; showTable?: boolean }) {
  return (
    <DataTableStateProvider storageKey="traces-table" defaultColumnOrder={defaultTracesColumnOrder}>
      <TracesTableContent showChart={showChart} showTable={showTable} />
    </DataTableStateProvider>
  );
}

function TracesTableContent({ showChart = true, showTable = true }: { showChart?: boolean; showTable?: boolean }) {
  ...
  const handleRowClick = useCallback(
    (row: Row<TraceRow>) => {
      ...
      const traceParams = new URLSearchParams(searchParams.toString());
      traceParams.delete("traceId");
      traceParams.delete("spanId");
      router.push(`/project/${projectId}/traces/${row.id}?${traceParams.toString()}`);
    },
  ...
  const getRowHref = useCallback(
    (row: Row<TraceRow>) => {
      ...
      return `/project/${projectId}/traces/${row.id}?${params.toString()}`;
    },
  ...
  const controls = (
    <>
      <div className="flex flex-1 pt-1 w-full h-full gap-2">
        {showTable && <DataTableFilter presetFilters={presetFilters} columns={filters} />}
        {showTable && (
          <ColumnsMenu
        ...
        {showTable && <SearchTracesInput />}
      </div>
      {showTable && <DataTableFilterList />}
      {showChart && (
        <div className="flex flex-col gap-4">
          <TracesMetrics traces={traces} />
          <TracesChart className="w-full border rounded-lg p-2 bg-[#121212]" containerRef={chartContainerRef} />
        </div>
      )}
    </>
  );
    ...
    return (
      <div className="flex flex-1 overflow-hidden px-4 pb-4">
        <div className="flex flex-col w-full h-full">
          <div className="flex pt-1 w-full gap-2 shrink-0">
            <DateRangeFilter />
            <RefreshButton onClick={handleRefresh} variant="outline" />
            <div className="flex items-center gap-2 px-2 border rounded-md bg-background h-7">
              <Switch id="realtime" checked={realtimeEnabled} onCheckedChange={setRealtimeEnabled} />
          ...
          {showChart && (
            <div className="flex-1 flex flex-col gap-2 min-h-0 pt-4">
              <TracesMetrics traces={traces} />
