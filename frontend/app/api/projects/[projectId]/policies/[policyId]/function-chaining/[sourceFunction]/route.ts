...
export async function DELETE(
  req: NextRequest,
  props: { params: Promise<{ projectId: string; policyId: string; sourceFunction: string }> }
): Promise<Response> {
  ...
  try {
    const { policyId, sourceFunction } = await props.params;
    ...
    const url = `${HIPOCAP_SERVER_URL}/api/v1/policies/${numericId}/function-chaining/${encodeURIComponent(sourceFunction)}`;
    ...
    console.error("Error deleting function chaining:", error);
