...
export async function DELETE(
  req: NextRequest,
  props: { params: Promise<{ projectId: string; policyId: string; roleName: string }> }
): Promise<Response> {
  ...
  try {
    const { policyId, roleName } = await props.params;
    ...
    const url = `${HIPOCAP_SERVER_URL}/api/v1/policies/${numericId}/roles/${encodeURIComponent(roleName)}`;
