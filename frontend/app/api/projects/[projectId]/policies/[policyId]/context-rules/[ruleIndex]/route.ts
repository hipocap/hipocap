...
export async function DELETE(
  req: NextRequest,
  props: { params: Promise<{ projectId: string; policyId: string; ruleIndex: string }> }
): Promise<Response> {
  ...
  try {
    const { policyId, ruleIndex } = await props.params;
    ...
    if (isNaN(index)) {
      return new Response(JSON.stringify({ error: "Invalid rule index. Must be a number." }), { status: 400 });
    }

    const url = `${HIPOCAP_SERVER_URL}/api/v1/policies/${numericId}/context-rules/${index}`;
