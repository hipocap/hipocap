...
export async function DELETE(
  req: NextRequest,
  props: { params: Promise<{ projectId: string; policyId: string; functionName: string }> }
): Promise<Response> {
  ...
  try {
    const { policyId, functionName } = await props.params;
    ...
    const url = `${HIPOCAP_SERVER_URL}/api/v1/policies/${numericId}/functions/${encodeURIComponent(functionName)}`;
