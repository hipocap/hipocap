...
export async function GET(
  ...
  try {
    ...
    const url = `${HIPOCAP_SERVER_URL}/api/v1/shields${ownerOnly ? "?owner_only=true" : ""}`;
...
export async function POST(
  ...
  try {
    ...
    const url = `${HIPOCAP_SERVER_URL}/api/v1/shields`;
    ...
    console.error("Error creating shield:", error);
    if (error instanceof ZodError) {
      return new Response(prettifyError(error), { status: 400 });
    }
    if (error instanceof Error) {
