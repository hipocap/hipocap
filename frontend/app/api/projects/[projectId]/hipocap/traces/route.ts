...
export async function GET(
  ...
  try {
    ...
    if (searchParams.get("function_name")) {
      queryParams.set("function_name", searchParams.get("function_name")!);
    }
    ...
    if (searchParams.get("final_decision")) {
      queryParams.set("final_decision", searchParams.get("final_decision")!);
    }
    ...
    const url = `${HIPOCAP_SERVER_URL}/api/v1/traces${queryParams.toString() ? `?${queryParams.toString()}` : ""}`;
    ...
    console.error("Error fetching Hipocap traces:", error);
