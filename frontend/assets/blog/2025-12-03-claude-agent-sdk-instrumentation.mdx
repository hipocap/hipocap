...
At **Hipocap**, we obsess over developer experience. We wanted to ensure that if you are building an agent on top of Claude, you can trace the entire execution flow, from your outer function to the inner workings of the Claude Node process, with zero friction.
...
Last week, agent developers asked us to instrument the SDK so that they could go through their Hipocap workflows while building up custom agents. They wanted to see:
...
The Claude Agent SDK allows changing `ANTHROPIC_BASE_URL`. Since Hipocap has a great integration with **LiteLLM** already, we thought we could point the SDK to a central LiteLLM proxy.
...
By solving the process-boundary problem with our Rust proxy, Hipocap now provides the only drop-in solution for tracing `claude-agent-sdk`.
...
True to our obsession with developer experience, we refused to make this complicated. The setup is just an install command and 1-2 lines of code. We respect the time of the researchers and engineers who rely on Hipocap. We want you analyzing your agents, not wrestling with your instrumentation.
...
from lmnr import Hipocap, observe
...
const query = Hipocap.wrapClaudeAgentQuery(origQuery);
async function run() {
