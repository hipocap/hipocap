...
Now you can see everything. With Hipocap, Stagehand, & Kernel, you have full observability for every agent decision and action, synced to a recording of what your agent actually saw and pondered.
...
**Debug when extract() returns malformed data.** LLMs still struggle to reliably produce valid JSON matching your schema. When `extract()` fails, open the trace, find the LLM span, and open the trace in a [Hipocap Playground](https://docs.lmnr.ai/playground/introduction). Hipocap loads the exact prompt, model config, and structured output schema. Tweak the prompt, adjust the schema definition, or try a different model. Iterate right there until it works, then update your code with confidence.

![Hipocap Playground with Stagehand extract pre-filled](/blog/2025-12-08-playground.gif)
...
Initialize Hipocap with Stagehand instrumentation:

```typescript
import { Hipocap } from "@lmnr-ai/lmnr";
...
Both workflows and Agent runs produce full traces in your Hipocap dashboard with synchronized browser recordings.

## What You Get

- **Session recordings synced to your traces** — Watch exactly what your agent saw, rendered as video. Hipocap captures the full browser session and synchronizes it with your execution timeline, so you can see the page state at every step. It's like a flight recorder for your browser automation.
